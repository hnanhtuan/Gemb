// Modifications by Ramin Raziperchikolaei 2015-01-21:
// The original file from the matlab/ directory of LIBLINEAR is train.c.old.
// - Changed the name of variable cross_validation_flag to
//   flag_cross_validation.
// - Defined a new variable: flag_cross_validation.
// - Copied function exit_with_help() from original train.c file in
//   liblinear-warmstart/ directory without modification.
// - Copied function parse_command_line() from original train.c file in
//   liblinear-warmstart/ directory and modified lines 223-225 and 255-265.
...
int parse_command_line(int nrhs, const mxArray *prhs[], char *model_file_name)
{
	...
	for(i=1;i<argc;i++)
	{
		...
		switch(argv[i-1][1])
		{
			...
			case 'i':
				...
				strcpy (str,"./BAtemp/");
				strcat (str,argv[i]);
				if((initial_model=load_model(str))==0)
				{
	...
	if(i>=argc)
		exit_with_help();
	if(i<argc-1)
	        sprintf(model_file_name,"./BAtemp/%s",argv[i+1]);
	else
	{
		...
		if(p==NULL)
			...
			++p;
		sprintf(model_file_name,"./BAtemp/%s",p);
	}
		...
		int nrhs, const mxArray *prhs[] )
{
	...
	if(nrhs > 1 && nrhs < 5)
	{
		...
		if(parse_command_line(nrhs, prhs, model_file_name))
		{
			exit_with_help();
			destroy_param(&param);
			fake_answer(nlhs, plhs);
